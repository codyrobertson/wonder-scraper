...
async def scrape_all_offers(slug: str, max_pages: int = 200, concurrency: int = 20) -> List[BlokpaxOffer]:
    ...
    Offers are attached to individual assets, so we need to:
    1. Get all asset IDs from bulk endpoint
    2. Fetch each asset's detail endpoint for offers
    ...
    Returns:
        List of all active BlokpaxOffer objects, sorted by price descending (highest bid first)
    """
    import aiohttp
    ...
    print(f"  Scanning {slug} for active offers...")

    async with aiohttp.ClientSession() as session:
        # Step 1: Collect all asset IDs from bulk endpoint
        all_asset_ids = []
        page = 1
        total_pages = 1
        total_assets = 0
        ...
        while page <= min(max_pages, total_pages):
            url = f"{BLOKPAX_API_BASE}/storefront/{slug}/assets"
            params = {"pg": page, "perPage": 100, "query": "", "sort": "price_asc"}
        ...
        print(f"  Found {len(all_asset_ids)} assets to check for offers")

        # Step 2: Probe each asset for offers using v2 API
        async def check_asset_offers(asset_id: str) -> List[BlokpaxOffer]:
            url = f"{BLOKPAX_API_BASE}/v2/storefront/{slug}/asset/{asset_id}"
            try:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status != 200:
                    ...
                    d = data.get("data", {})
                    asset_info = d.get("asset", {})
                    offers_data = asset_info.get("offers", [])
                    asset_name = d.get("name", "Unknown")

                    found_offers = []
                    for offer in offers_data:
                        # Only get open offers
                        if offer.get("offer_status") != "open":
                            continue

                        raw_price = offer.get("offer_bpx_per_token", 0)
                        if raw_price > 0:
                            offer_id = str(offer.get("id", ""))
                            found_offers.append(BlokpaxOffer(
                                offer_id=offer_id,
                                asset_id=str(asset_id),
                                price_bpx=bpx_to_float(raw_price),
                                price_usd=bpx_to_usd(raw_price, bpx_price),
                    ...
                    if found_offers:
                        best_offer = max(found_offers, key=lambda x: x.price_bpx)
                        print(f"    ✓ {asset_name[:35]} has {len(found_offers)} offer(s), best: {best_offer.price_bpx:,.0f} BPX")
        ...
        # Step 3: Probe assets in concurrent batches
        checked = 0
        total_to_check = len(all_asset_ids)

        for i in range(0, total_to_check, concurrency):
            batch = all_asset_ids[i:i + concurrency]
            tasks = [check_asset_offers(aid) for aid in batch]
            results = await asyncio.gather(*tasks)
            ...
            checked += len(batch)
            if checked % 200 == 0 or checked == total_to_check:
                print(f"    Progress: {checked}/{total_to_check} assets checked, {len(all_offers)} offers found")

            await asyncio.sleep(0.05)

    # Sort by price descending (highest bid first)
    all_offers.sort(key=lambda x: x.price_bpx, reverse=True)
    print(f"  Done! Found {len(all_offers)} active offers")
    return all_offers
